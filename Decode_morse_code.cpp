...
std::string decodeMorse(std::string morseCode) {
    std::string decoded;
    if (!morseCode.empty()) morseCode.erase(0, morseCode.find_first_not_of(" "));
    while (!morseCode.empty()) {
      if (morseCode.compare(0, 1, " ") != 0) {
        size_t buf =  morseCode.find_first_of(" ");
        decoded += MORSE_CODE [morseCode.substr(0, buf)];
        morseCode.erase(0, morseCode.size() == 1 ? std::string::npos 
                                                  : (buf != std::string::npos 
                                                      ? buf + 1 : buf));
      }
      else { 
        size_t buf = morseCode.find_first_not_of(" ");
        morseCode.erase(0, buf);
        if (!morseCode.empty()) decoded += " "; 
      }
    }
    return decoded;
}